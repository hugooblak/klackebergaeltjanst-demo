 ...
 * or photo. They call kl_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Kläckeberga Eltjänst AB.
 *
 ...
 * @return string
 */
function kl_sample_note() {
	return kl_lead( 'ar_riktig' ) ? '' : ' <span class="kl-sample">(platshållare)</span>';
}
 ...
 * @return string File name for kl_lead_img().
 */
function kl_lead_bild( $i ) {
	$bilder = (array) kl_lead( 'bilder', array() );
 ...
 * @return string
 */
function kl_ring_label() {
	$namn = kl_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, kl_lead( 'telefon' ) ) : 'Ring ' . kl_lead( 'telefon' );
}
