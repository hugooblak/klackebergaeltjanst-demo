...
add_filter(
	...
	function ( $exporters ) {
		$exporters['klackebergaeltjanst-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'klackebergaeltjanst-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'klackebergaeltjanst-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'klackebergaeltjanst-leads',
						'group_label' => __( 'Quote requests', 'klackebergaeltjanst-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['klackebergaeltjanst-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'klackebergaeltjanst-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
