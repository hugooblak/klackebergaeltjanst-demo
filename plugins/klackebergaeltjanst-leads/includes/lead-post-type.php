...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'klackebergaeltjanst-leads' ),
					'singular_name' => __( 'Lead', 'klackebergaeltjanst-leads' ),
					'edit_item'     => __( 'Förfrågan', 'klackebergaeltjanst-leads' ),
					'search_items'  => __( 'Search leads', 'klackebergaeltjanst-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'klackebergaeltjanst-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'klackebergaeltjanst-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'klackebergaeltjanst-leads' ),
			'npl_phone'    => __( 'Phone', 'klackebergaeltjanst-leads' ),
			'npl_service'  => __( 'Needs', 'klackebergaeltjanst-leads' ),
			'npl_timeline' => __( 'When', 'klackebergaeltjanst-leads' ),
			'npl_zip'      => __( 'ZIP', 'klackebergaeltjanst-leads' ),
			'npl_source'   => __( 'Source', 'klackebergaeltjanst-leads' ),
			'npl_status'   => __( 'Status', 'klackebergaeltjanst-leads' ),
			'date'         => __( 'Received', 'klackebergaeltjanst-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'klackebergaeltjanst-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'klackebergaeltjanst-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'klackebergaeltjanst-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'klackebergaeltjanst-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'klackebergaeltjanst-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'klackebergaeltjanst-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'klackebergaeltjanst-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'klackebergaeltjanst-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'klackebergaeltjanst-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'klackebergaeltjanst-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'klackebergaeltjanst-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'klackebergaeltjanst-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
